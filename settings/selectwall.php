<?
    ...
    if($_COOKIE['language']=='english'){include("translater.php");
        ...
        if($type==0 or $type==3){$res = mysql_query('SELECT COUNT(1) FROM `wall` WHERE `user` = '.$id.'');}
        elseif($type==1){$res = mysql_query('SELECT COUNT(1) FROM `wall` WHERE `user` = '.$id.' and `w_hidden` = 0');}
        elseif($type==2){$res = mysql_query('SELECT COUNT(1) FROM `wall` WHERE `user` = '.$id.' and `w_hidden` = 1');} 
        if($res)
        ...
        $his_posts = !empty($row[0]) ? $row[0] : 0;
            ...
            echo"</div>";
                    ...
                    if($type==0){$result = mysql_query("SELECT * FROM `wall` WHERE `user` = '".$id."' ORDER BY id DESC");}
                    elseif($type==1){$result = mysql_query("SELECT * FROM `wall` WHERE `user` = '".$id."' AND w_hidden=0 ORDER BY id DESC");}
                    elseif($type==2){$result = mysql_query("SELECT * FROM `wall` WHERE `user` = '".$id."' AND w_hidden=1 ORDER BY id DESC");}
                    //elseif($type==3){$result = mysql_query("SELECT * FROM  `wall` WHERE `user` =".$id."  ORDER BY id DESC LIMIT {$count}");}
                    if(mysql_num_rows($result)==0){echo'<div class="wall_post_noposts">There are no posts yet.</div>';}
                    while($row = mysql_fetch_array($result)) {
                        $w_id=$row['id'];
                        $w_text=$row['text'];
                        $w_date=$row['w_date'];
    ...
    <div class="wall_post show">
                <div class="wall_post_avatar">';
                    ...
                    while($row = mysql_fetch_array($zapros1)) {$get_uavatar_path=$row['path'];} echo'
                ...
                <div class="wall_post_stats">
                    <ul>
                    <a class="wall_post_stats_like'; $get_likeinfo = mysql_query("SELECT `log_id` FROM `likes` WHERE `who_liked` = '$user_id' AND `owner` = '$id' AND `object_id` = '$w_id' AND `type` = '3'");if(mysql_num_rows($get_likeinfo)!=0){echo' liked';} echo' wlbid'.$w_id.'" type="3" object_id="'.$w_id.'" owner="'.$id.'">
                    <li>';
                        //check wall likes
                        $res = mysql_query('SELECT COUNT(1) FROM `likes` WHERE `owner` = '.$id.' AND type = 3 AND object_id = '.$w_id.'');
                        if($res)
                        $row = mysql_fetch_array($res, MYSQL_NUM);
    ...
    <path d="M10.1 15.55L10 15.65L9.89 15.55C5.14 11.24 2 8.39 2 5.5C2 3.5 3.5 2 5.5 2C7.04 2 8.54 3 9.07 4.36H10.93C11.46 3 12.96 2 14.5 2C16.5 2 18 3.5 18 5.5C18 8.39 14.86 11.24 10.1 15.55ZM14.5 0C12.76 0 11.09 0.81 10 2.08C8.91 0.81 7.24 0 5.5 0C2.42 0 0 2.41 0 5.5C0 9.27 3.4 12.36 8.55 17.03L10 18.35L11.45 17.03C16.6 12.36 20 9.27 20 5.5C20 2.41 17.58 0 14.5 0Z"/>
                            ...
                            <span>';$get_likeinfo = mysql_query('SELECT `log_id` FROM `likes` WHERE `who_liked` = '.$user_id.' AND `owner` = '.$id.' AND `object_id` = '.$w_id.' AND `type` = 3');if(mysql_num_rows($get_likeinfo)!=0){echo'Liked';}else{echo'Like';} echo' (';$wallike_count=str_split($his_wall_likes);if($his_wall_likes<1000){echo$his_wall_likes;}elseif($his_wall_likes<10000){echo$wallike_count[0].'K';}elseif($his_wall_likes<100000){echo$wallike_count[0].$wallike_count[1].'K';}elseif($his_wall_likes<1000000){echo$wallike_count[0].$wallike_count[1].$wallike_count[2].'K';}elseif($his_wall_likes<10000000){echo$wallike_count[0].'KK';}else{$his_wall_likes;} echo')</span>
                        </li></a>
                        <li>
