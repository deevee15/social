<?
    ...
    $id=$_GET['id'];
    //$id=$_POST['id'];
    $check_online = mysql_query("SELECT online FROM users WHERE id='$id'");
    $get_result = mysql_fetch_array($check_online);
    if($get_result['online']==1){echo'online';}
    else{
        $result = mysql_query("SELECT * FROM `users` WHERE id = '$id'");
        while($row = mysql_fetch_array($result)) {$last_login=$row['last_login'];}
