...
function playPause(){
    ...
    if(activeSong.paused){
         activeSong.play();
         $('#songPlayPause').css({"background":"#42aaff url(/img/player.png) 0px -55px"});
    }else{
         activeSong.pause();
         $('#songPlayPause').css({"background":"#42aaff url(/img/player.png) 0px 0px"});
    }
}
...
function changeVolume(number, direction){
    ...
    if(activeSong.volume >= 0 && direction == "down"){
        $(this).volume = $(this).volume - (number / 100);
    }
